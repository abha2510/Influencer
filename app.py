...
def register_user():
    ...
    users_collection.insert_one(
        {"username": username, "password": password, "email": email, "chats": []}
    )
...
They can’t do it. We have to do it for them.That is our responsibility as a parent. 
Our #1 job. Because if you see what is parenting: parenting is is interference in child’s life. Just that we generally spend too much time interfering on the wrong things: what to wear, what not to wear, combing their hair etc. But there are places where we need to interfere
...
def check_for_age(question):
    ...
    if any(keyword in question for keyword in age_keywords): 
        return True
    else:
        False

def check_for_problem(question):
    problem_keywords = [
        "problem",
        "how",
        "why",
...
def chat():
    ...
    missing_info = []   # Define missing_info here
    if not has_already_sent_message:
        if not check_for_age(q):
            missing_info.append("the age of your child")
        if not check_for_problem(q):
            missing_info.append("more details about the issue you're facing")
    if not missing_info:  
        if not is_parenting_related(q):
            return jsonify(
                {
                    "chatbot_response": "Your first question should be related to parenting."
                }
            )
        else:
    ...
    else:  # if there's some missing info
        result = "Could you please provide " + ", and ".join(missing_info) + "?"
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a helpful parent influencer that speaks the same language as the user.",
        }
    ]
